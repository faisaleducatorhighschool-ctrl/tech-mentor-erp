...
router.get("/dashboard/stats", requireAuth, async (_req, res): Promise<void> => {
  ...
  const [salesStats] = await db.select({
    todaySales: sql<number>`coalesce(sum(case when created_at >= CURRENT_DATE then (case when is_return then -1 else 1 end) * total_amount::numeric else 0 end), 0)`,
    monthSales: sql<number>`coalesce(sum(case when created_at >= date_trunc('month', CURRENT_DATE) then (case when is_return then -1 else 1 end) * total_amount::numeric else 0 end), 0)`,
    totalRevenue: sql<number>`coalesce(sum((case when is_return then -1 else 1 end) * total_amount::numeric), 0)`,
...
router.get("/dashboard/sales-chart", requireAuth, async (_req, res): Promise<void> => {
  const rows = await xr(sql`
    ...
    LEFT JOIN sales s ON s.created_at::date = days.day
